...
#[test]
fn can_hold_value() {
    let mut storage_2 = [0u8; 3];
    let mut vec_2 = StackVec::new(&mut storage_2);
    vec_2.push(13).unwrap();
    vec_2.push(15).unwrap();
    assert_eq!(15u8, vec_2.pop().unwrap())
}
#[test]
fn assignment_text_example() {
    ...
    // possibly failing because deref is not working properly
